...
import postgresql16 from '@/api/plugins/postgresql16'
import { generateRandomString, renderIcon } from '@/utils'
import type { Backup, Database, User } from '@/views/plugins/postgresql16/types'
...
const getLoad = async () => {
  const { data } = await postgresql16.load()
  return data
}

const getDatabaseList = async (page: number, limit: number) => {
  const { data } = await postgresql16.databases(page, limit)
  return data
}

const getUserList = async (page: number, limit: number) => {
  const { data } = await postgresql16.users(page, limit)
  return data
}

const getBackupList = async (page: number, limit: number) => {
  const { data } = await postgresql16.backups(page, limit)
  return data
}
...
const handleDeleteDatabase = async (name: string) => {
  postgresql16.deleteDatabase(name).then(() => {
...
const handleDeleteUser = async (user: string) => {
  postgresql16.deleteUser(user).then(() => {
    window.$message.success('删除成功')
...
const getStatus = async () => {
  await postgresql16.status().then((res: any) => {
    status.value = res.data
  })
}

const getLog = async () => {
  const { data } = await postgresql16.log()
  return data
}

const getConfig = async () => {
  postgresql16.config().then((res: any) => {
    config.value = res.data
  })
}

const getUserConfig = async () => {
  postgresql16.userConfig().then((res: any) => {
    userConfig.value = res.data
  })
}

const handleSaveConfig = async () => {
  await postgresql16.saveConfig(config.value)
  window.$message.success('保存成功')
}

const handleSaveUserConfig = async () => {
  await postgresql16.saveUserConfig(userConfig.value)
  window.$message.success('保存成功')
}

const handleClearLog = async () => {
  await postgresql16.clearLog()
  getLog().then((res) => {
    log.value = res
  })
...
const handleStart = async () => {
  await postgresql16.start()
  window.$message.success('启动成功')
  await getStatus()
}

const handleStop = async () => {
  await postgresql16.stop()
  window.$message.success('停止成功')
  await getStatus()
}

const handleRestart = async () => {
  await postgresql16.restart()
  window.$message.success('重启成功')
  await getStatus()
}

const handleReload = async () => {
  await postgresql16.reload()
  window.$message.success('重载成功')
  await getStatus()
}

const handleAddDatabase = async () => {
  postgresql16.addDatabase(addDatabaseModel.value).then(() => {
...
const handleAddUser = async () => {
  postgresql16.addUser(addUserModel.value).then(() => {
    window.$message.success('添加成功')
...
const handleUploadBackup = async (files: UploadFileInfo[]) => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    await postgresql16.uploadBackup(formData).then(() => {
...
const handleCreateBackup = async () => {
  ...
  await postgresql16.createBackup(currentDatabase.value).then(() => {
    messageReactive?.destroy()
...
const handleRestoreBackup = async (row: any) => {
  ...
  await postgresql16.restoreBackup(row.name, currentDatabase.value).then(() => {
    messageReactive?.destroy()
...
const handleDeleteBackup = async (name: string) => {
  await postgresql16.deleteBackup(name).then(() => {
    window.$message.success('删除成功')
    onBackupPageChange(backupPagination.page)
  })
}
