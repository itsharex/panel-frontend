...
const getLoad = async () => {
  const { data } = await mysql84.load()
  return data
}

const getDatabaseList = async (page: number, limit: number) => {
  const { data } = await mysql84.databases(page, limit)
  return data
}

const getUserList = async (page: number, limit: number) => {
  const { data } = await mysql84.users(page, limit)
  return data
}

const getBackupList = async (page: number, limit: number) => {
  const { data } = await mysql84.backups(page, limit)
  return data
}
...
const handleDeleteDatabase = async (name: string) => {
  mysql84.deleteDatabase(name).then(() => {
...
const getRootPassword = async () => {
  await mysql84.rootPassword().then((res: any) => {
    rootPassword.value = res.data
  })
}

const getErrorLog = async () => {
  const { data } = await mysql84.errorLog()
  return data
}

const getSlowLog = async () => {
  const { data } = await mysql84.slowLog()
  return data
}

const getConfig = async () => {
  const { data } = await mysql84.config()
  return data
}

const handleSaveConfig = async () => {
  await mysql84.saveConfig(config.value)
  window.$message.success('保存成功')
}

const handleClearErrorLog = async () => {
  await mysql84.clearErrorLog()
...
const handleSetRootPassword = async () => {
  await mysql84.setRootPassword(rootPassword.value)
  window.$message.success('修改成功')
}

const handleAddDatabase = async () => {
  mysql84.addDatabase(addDatabaseModel.value).then(() => {
...
const handleAddUser = async () => {
  mysql84.addUser(addUserModel.value).then(() => {
    window.$message.success('添加成功')
...
const handleUploadBackup = async (files: UploadFileInfo[]) => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    await mysql84.uploadBackup(formData).then(() => {
...
const handleCreateBackup = async () => {
  ...
  await mysql84.createBackup(currentDatabase.value).then(() => {
    messageReactive?.destroy()
...
const handleRestoreBackup = async (row: any) => {
  ...
  await mysql84.restoreBackup(row.name, currentDatabase.value).then(() => {
    messageReactive?.destroy()
...
const handleDeleteBackup = async (name: string) => {
  await mysql84.deleteBackup(name).then(() => {
    window.$message.success('删除成功')
    onBackupPageChange(backupPagination.page)
  })
}
