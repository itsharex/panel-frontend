...
import php82 from '@/api/plugins/php82'
import { renderIcon } from '@/utils'
import Editor from '@guolao/vue-monaco-editor'

const currentTab = ref('status')
const version = ref('8.2')
...
const getLoad = async () => {
  const { data } = await php82.load()
  return data
}

const getExtensions = async () => {
  const { data } = await php82.extensions()
  return data
}

const getStatus = async () => {
  await php82.status().then((res: any) => {
    status.value = res.data
  })
}

const getErrorLog = async () => {
  php82.errorLog().then((res: any) => {
    errorLog.value = res.data
  })
}

const getSlowLog = async () => {
  php82.slowLog().then((res: any) => {
    slowLog.value = res.data
  })
}

const getConfig = async () => {
  php82.config().then((res: any) => {
    config.value = res.data
  })
}

const getFPMConfig = async () => {
  php82.fpmConfig().then((res: any) => {
    fpmConfig.value = res.data
  })
}

const handleSaveConfig = async () => {
  await php82.saveConfig(config.value)
  window.$message.success('保存成功')
  await getErrorLog()
}

const handleSaveFPMConfig = async () => {
  await php82.saveFPMConfig(fpmConfig.value)
  window.$message.success('保存成功')
  await getFPMConfig()
}

const handleClearErrorLog = async () => {
  await php82.clearErrorLog()
  await getErrorLog()
  window.$message.success('清空成功')
}

const handleClearSlowLog = async () => {
  await php82.clearSlowLog()
  await getSlowLog()
  window.$message.success('清空成功')
}
