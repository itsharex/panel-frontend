...
import php81 from '@/api/plugins/php81'
...
const getLoad = async () => {
  const { data } = await php81.load()
  return data
}

const getExtensions = async () => {
  const { data } = await php81.extensions()
  return data
}

const getStatus = async () => {
  await php81.status().then((res: any) => {
    status.value = res.data
  })
}

const getErrorLog = async () => {
  php81.errorLog().then((res: any) => {
    errorLog.value = res.data
  })
}

const getSlowLog = async () => {
  php81.slowLog().then((res: any) => {
    slowLog.value = res.data
  })
}

const getConfig = async () => {
  php81.config().then((res: any) => {
...
const handleClearErrorLog = async () => {
  await php81.clearErrorLog()
  await getErrorLog()
  window.$message.success('清空成功')
}

const handleClearSlowLog = async () => {
  await php81.clearSlowLog()
  await getSlowLog()
  window.$message.success('清空成功')
}
