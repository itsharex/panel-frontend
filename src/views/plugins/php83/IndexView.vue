...
import php83 from '@/api/plugins/php83'
...
const getLoad = async () => {
  const { data } = await php83.load()
  return data
}

const getExtensions = async () => {
  const { data } = await php83.extensions()
  return data
}

const getStatus = async () => {
  await php83.status().then((res: any) => {
    status.value = res.data
  })
}

const getErrorLog = async () => {
  php83.errorLog().then((res: any) => {
    errorLog.value = res.data
  })
}

const getSlowLog = async () => {
  php83.slowLog().then((res: any) => {
    slowLog.value = res.data
  })
}

const getConfig = async () => {
  php83.config().then((res: any) => {
    config.value = res.data
  })
}

const getFPMConfig = async () => {
  php83.fpmConfig().then((res: any) => {
    fpmConfig.value = res.data
  })
}

const handleSaveConfig = async () => {
  await php83.saveConfig(config.value)
  window.$message.success('保存成功')
  await getErrorLog()
}

const handleSaveFPMConfig = async () => {
  await php83.saveFPMConfig(fpmConfig.value)
  window.$message.success('保存成功')
  await getFPMConfig()
}

const handleClearErrorLog = async () => {
  await php83.clearErrorLog()
  await getErrorLog()
  window.$message.success('清空成功')
}

const handleClearSlowLog = async () => {
  await php83.clearSlowLog()
  await getSlowLog()
  window.$message.success('清空成功')
}
