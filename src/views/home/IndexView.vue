...
<template>
  <AppPage :show-footer="true" min-w-375>
    <div flex-1>
      <n-space vertical>
        <n-grid x-gap="12" y-gap="12" cols="1 s:1 m:1 l:3 xl:3 2xl:3" item-responsive responsive="screen">
          <n-gi>
            <n-card :segmented="true" rounded-10 size="small" title="资源使用">
              <n-space v-if="nowMonitor" vertical :size="30">
                ...
                <n-thing v-if="nowMonitor">
                  ...
                  <p>物理内存 使用 {{ formatBytes(nowMonitor.mem.used) }} / 总共 {{
                      formatBytes(nowMonitor.mem.total)
                    }}</p>
                  <p>交换分区 使用 {{ formatBytes(nowMonitor.swap.used) }} / 总共
                    {{ formatBytes(nowMonitor.swap.total) }}</p>
                </n-thing>
              </n-space>
              ...
              <n-space v-if="nowMonitor" vertical size="large">
                <n-thing>
                  ...
                  <n-popover trigger="hover" placement="top-end">
                    ...
                    <span>1 分钟负载 <n-tag type="primary">{{ nowMonitor.load.load1 }}</n-tag></span>
                  </n-popover>
                </n-thing>
                <n-thing>
                  <template #avatar>
                  ...
                  <n-popover trigger="hover" placement="top-end">
                    ...
                    <span>5 分钟负载 <n-tag type="primary">{{ nowMonitor.load.load5 }}</n-tag></span>
                  </n-popover>
                </n-thing>
                <n-thing>
                  <template #avatar>
                  ...
                  <n-popover trigger="hover" placement="top-end">
                    ...
                    <span>15 分钟负载 <n-tag type="primary">{{ nowMonitor.load.load15 }}</n-tag></span>
                  </n-popover>
                </n-thing>
              </n-space>
              <n-skeleton v-else text :repeat="10"/>
            ...
            <n-card :segmented="true" rounded-10 size="small" title="实时流量">
              <n-space v-if="nowMonitor" vertical :size="36">
                <n-thing>
                  ...
                  <p>实时上行 {{ formatBytes(netCurrentSent) }}/s / 实时下行 {{ formatBytes(netCurrentRecv) }}/s</p>
                  <p>累计上行 {{ formatBytes(netTotalSent) }} / 累计下行 {{ formatBytes(netTotalRecv) }}</p>
                </n-thing>
                <n-thing>
                  <template #avatar>
                    <n-avatar>
                  ...
                  <p>实时读取 {{ formatBytes(diskCurrentRead) }}/s / 实时写入 {{ formatBytes(diskCurrentWrite) }}/s</p>
                  <p>累计读取 {{ formatBytes(diskTotalRead) }} / 累计写入 {{ formatBytes(diskTotalWrite) }}</p>
                </n-thing>
              </n-space>
              <n-skeleton v-else text :repeat="10"/>
            </n-card>
        ...
        <div>
          <n-card :segmented="true" rounded-10 size="small" title="概览">
            <template #header-extra>
              <n-button text type="primary"> 更多</n-button>
            </template>
          </n-card>
        </div>
        <n-grid x-gap="12" y-gap="12" cols="1 s:1 m:2 l:3 xl:3 2xl:3" item-responsive responsive="screen">
          <n-gi span="2 s:1 m:1 l:2">
            <div flex-1 min-w-375>
              <n-card :segmented="true" rounded-10 size="small" title="存储信息">
                <n-space v-if="nowMonitor" class="pt-10 pb-10">
          ...
          <n-gi span="2 s:1 m:1 l:2">
            <div flex-1 min-w-375>
              <n-card :segmented="true" rounded-10 size="small" title="快捷插件">
                <n-grid v-if="homePlugins" x-gap="12" y-gap="12" cols="3 s:1 m:2 l:3" item-responsive
                        responsive="screen">
